    ...
    def export_key(self):
        if platform == 'android':
            ...
            try:
                ...
                save_file = [os.path.join(os.path.expanduser('~'), 'Downloads', 'key_file')]
                self.dialog = MDDialog(
                    title="Экспорт ключа",
                    text="Ключ успешно экспортирован в ~/downloads/key_file"
                )
                self.dialog.open()
    ...
    def upload_avatar(self):
        if platform == 'android':
            ...
            try:
                ...
                image = [os.path.join(os.path.expanduser('~'), 'Downloads', 'image.png')]

                if not os.path.exists(image[0]):
                    self.dialog = MDDialog(
                        title="Файл не найден",
                        text="Поместите файл с картинкой в ~/Downloads/image.png"
                    )
                    self.dialog.open()
                else:
                    self.true_upload_avatar(image, False)
